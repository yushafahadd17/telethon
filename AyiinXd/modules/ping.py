...
async def _(ping):
    ...
    await Ayiin.edit(
        f"[ᴀᴇᴛʜᴇʀ-ᴜꜱᴇʀʙᴏᴛ](tg://user?id=6442877226)\n\n"
        f"sᴘᴇᴇᴅ: `%sms`\n"
        f"ᴜᴘᴛɪᴍᴇ : `{uptime}` \n"
...
async def _(store):
    datetime.now()
    xx = await edit_or_reply(store, "https://t.me/yushaa17")

@ayiin_cmd(pattern="kiboy$")
async def _(kiboy):
    datetime.now()
    xx = await edit_or_reply(kiboy, "https://t.me/yushaa17")

@ayiin_cmd(pattern="nokos$")
async def _(nokos):
    datetime.now()
    xx = await edit_or_reply(nokos, "https://t.me/yushaa17")
